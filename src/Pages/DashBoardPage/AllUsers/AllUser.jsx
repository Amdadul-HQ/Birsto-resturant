...
const AllUser = () => {
    ...
    const handleUpdate = async(id,name)=> {
        const swalWithBootstrapButtons = Swal.mixin({
          ...
          swalWithBootstrapButtons.fire({
            title: "Are you sure?",
            text: `You want to make Admin ${name}?`,
            icon: "warning",
            showCancelButton: true,
...
<div className="flex items-center justify-between gap-x-3">
    ...
    <h2 className="text-lg font-medium text-gray-800 dark:text-white">Total User</h2>
            ...
            <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                        {
                            users && users.map((item,index)=> <tr key={item._id}>
                                ...
                                <td >
                                    {
                                        item.role == 'Admin' ? <div className="inline-flex items-center px-3 py-1 rounded-full gap-x-2 bg-emerald-100/60 dark:bg-gray-800">
                                        <span className="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>

                                        <h2 className="text-sm font-normal text-emerald-500">Admin</h2>
                                    </div> : <button
                                    
                                    onClick={()=>handleUpdate(item._id,item.name)}
                                    className="px-4 py-4  text-gray-500 dark:text-gray-300 whitespace-nowrap text-sm">
                                   <FaUsers className="bg-[#D1A054] p-2 rounded-md text-xl text-white w-12 h-12"/>
                                   </button>
                                    }
                                    </td>
