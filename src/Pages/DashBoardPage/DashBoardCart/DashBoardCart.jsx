            ...
            if (result.isConfirmed) {
                ...
                .then(res => {
                    if(res.data.deletedCount>0){
                        toast.success(`${name} is Delete From Cart`)
                    }
                })
                refetch()
            //   swalWithBootstrapButtons.fire({
    ...
    return (
        <section className="container mx-auto">
            <div className='flex w-full justify-center mt-5 mb-6'>
                <SectionHeading title='WANNA ADD MORE?' subTitle='My Cart'></SectionHeading>
            </div>
            <div>
            <section className="max-w-screen-lg px-4 mx-auto">
    <div className="flex items-center justify-between gap-x-3">
        <div className="flex items-center gap-x-4">
        <h2 className="text-lg font-medium text-gray-800 dark:text-white">Total Order</h2>
        ...
        <div className="flex items-center gap-x-4">
            ...
            <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full dark:bg-gray-800 dark:text-blue-400">{totalPrice}</span>
        </div>
        <div>
            <button className="text-lg font-medium  text-white bg-orange-400 px-5 py-1 rounded-lg">Pay</button>
        </div>
    </div>
                                ...
                                cart && cart.map(item => <tr key={item._id}>
                                    ...
                                    <td className="px-4 py-4 text-sm whitespace-nowrap">
                                        <div className="flex items-center gap-x-6">
                                            <button onClick={()=>handledelete(item._id,item.name)} className="text-gray-500 transition-colors duration-200 dark:hover:text-red-500 dark:text-gray-300 hover:text-red-500 focus:outline-none">
